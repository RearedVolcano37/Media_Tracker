...
I chose to implement the Media Tracker component for my proof-of-concept because it strongly aligns with my personal interests while remaining realistic to complete within the semester. I regularly read manga, manhua, webtoons, and watch anime, so this is a system I would personally use, which increases my motivation to design it well. From a feasibility standpoint, the component primarily involves storing structured entries and updating fields such as progress and status, which makes it manageable in scope without requiring complex algorithms. Additionally, the design naturally separates minimal storage operations from higher-level reporting and filtering features, making it well-suited for later adaptation into the OSU software sequence discipline with kernel and secondary interfaces. For these reasons, it is both personally meaningful and practically achievable.
